...
# ================= CONFIG =================
TOKEN = os.getenv("BOT_TOKEN")   # token from cloud env
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "1️⃣ Send reference signatures (you can send multiple)\n"
        "2️⃣ Type /verify when done\n"
        "3️⃣ Send TEST signature\n\n"
...
async def save_reference(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("⚠️ Network issue. Please resend image.")
        return REFERENCE
...
async def test_image(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for file in os.listdir(user_dir):
        ...
        best_score = max(best_score, (0.7 * s) + (0.3 * m))
